...
export type AgentAction =
  | { type: 'scroll'; target: string; tab?: string }
  | { type: 'openBlogList' }
...
export const intentTabs: Partial<Record<string, string>> = {
  current_role: 'journey',
  career_timeline: 'journey',
  devops_stage: 'journey',
  aiops_stage: 'journey',
  founders_office: 'journey',
  skills: 'roles',
  general_about: 'roles',
  education: 'journey',
}
...
export const knowledgeSummary = {
  now: `${identity.name.split(' ')[0]} works in the ${identity.role} at ${identity.company}, in the seat since August 2026. The path ran DevOps Engineer (until March 2026), then AIOps Engineer (March to July 2026), then Technical Founder's Office. The short version: infrastructure reliability, then AI operations, now technical strategy and execution alongside the founder.`,
  career: `Three stages at ${identity.company}: DevOps Engineer (Feb 2025 to Mar 2026) holding 99.9% uptime across AWS/Azure/GCP with 5+ Kubernetes clusters and 200+ Terraform-managed resources; AIOps Engineer (Mar to Jul 2026) running LLM & RAG infrastructure at 1,000+ daily requests and building self-healing systems; Technical Founder's Office (Aug 2026 to present) on technical strategy and execution.`,
  ...
  about: `${identity.name}, ${identity.role} at ${identity.company}, based in ${identity.location}. B.Tech in Computer Science from JIET Jodhpur (2020 to 2024). The through-line: find the constraint, build the system that removes it, from uptime and FinOps to self-healing agents to company-level technical decisions.`,
...
export function buildReplies(): Record<string, AgentReply> {
  return {
    current_role: {
      text: `Right now ${identity.shortName} is in the ${currentStage.role} at ${identity.company}, in the seat since August 2026. Before that: AIOps Engineer (Mar to Jul 2026) and DevOps Engineer (Feb 2025 to Mar 2026). The progression is intentional: reliability, then AI operations, then founder-office execution.`,
      action: { type: 'scroll', target: 'about' },
      chips: [
        { label: 'Show the full timeline', action: { type: 'scroll', target: 'about' } },
        { label: 'What does he work on?', action: { type: 'scroll', target: 'about' } },
      ],
    },
    career_timeline: {
      text: `Three stages, one direction:\n\n1 · DevOps Engineer, Feb 2025 to Mar 2026. 99.9% uptime across AWS, Azure, GCP; 5+ Kubernetes clusters; 200+ resources as code; deploy frequency up 40%; cloud spend down 20%.\n\n2 · AIOps Engineer, Mar to Jul 2026. LLM & RAG infrastructure at 1,000+ daily requests; self-healing CI/CD; MCP server for 15+ AWS accounts; MTTR down 30%.\n\n3 · Technical Founder's Office, Aug 2026 to now. Technical strategy and execution alongside the founder.`,
      action: { type: 'scroll', target: 'about' },
      chips: [
        { label: 'Show DevOps stage', action: { type: 'scroll', target: 'about' } },
        { label: 'Show his projects', action: { type: 'openProject', projectId: projects[0].id } },
      ],
    },
    devops_stage: {
      text: `As DevOps Engineer at ${identity.company} (Feb 2025 to Mar 2026) he ran multi-client infrastructure: 99.9% uptime across AWS, Azure, and GCP; 5+ Kubernetes clusters (EKS, GKE) with 50+ microservices; 200+ Terraform-managed resources; GitOps raising deploy frequency 40%; FinOps cutting spend 20%; observability dropping MTTD 50%.`,
      action: { type: 'scroll', target: 'about' },
    },
    aiops_stage: {
      text: `As AIOps Engineer (Mar to Jul 2026) he deployed LLM & RAG infrastructure sustaining 1,000+ daily API requests for GenAI workloads, built the self-healing CI/CD agent and the Anthropic MCP inventory server, and cut MTTR 30% through SOPs and knowledge-sharing loops.`,
      action: { type: 'scroll', target: 'about' },
    },
    founders_office: {
    ...
    projects: {
      ...
      chips: [
        ...
        { label: 'See his stack', action: { type: 'scroll', target: 'about', tab: 'roles' } },
      ],
    },
    certifications: {
    ...
    recognition: {
      text: `Two highlights: winner of Ignition (Shellkode's internal hackathon, 2026) and Claude Architect certified by Anthropic (2026). Earlier: President of the JIET Student Council, where he led 150+ members and secured ₹20 Lakh in sponsorships.`,
      action: { type: 'scroll', target: 'credentials' },
    },
    blog_list: {
    ...
    general_about: {
      text: `${identity.name}, ${identity.role} at ${identity.company}, based in ${identity.location}. B.Tech in Computer Science from JIET Jodhpur (2020 to 2024). The through-line across everything: find the constraint, build the system that removes it, from 99.9% uptime and FinOps, to self-healing agents, to company-level technical decisions.`,
      action: { type: 'scroll', target: 'about' },
      chips: [
        { label: 'Career journey', action: { type: 'scroll', target: 'about' } },
        { label: 'Selected projects', action: { type: 'scroll', target: 'work' } },
      ],
    },
    education: {
      text: `B.Tech in Computer Science from JIET Jodhpur (Aug 2020 to May 2024), GPA 8.8/10. Along the way: President of the Student Council (150+ members, ₹20 Lakh in sponsorships) and Secretary of the CSE Department.`,
      action: { type: 'scroll', target: 'about' },
    },
    fallback: {
      text: knowledgeSummary.fallback,
      chips: [
        { label: 'What is he doing now?', action: { type: 'scroll', target: 'about' } },
...
export const promptChips = [
  { label: 'What is Bhavesh doing now?', action: { type: 'scroll', target: 'about' } as AgentAction },
